...
interface VmSafe {
    ...
    /// An RPC URL and its aliasName. Returned by `rpcUrlStructs`.
    struct Rpc {
        // The aliasName of the RPC URL.
        string key;
        // The RPC URL.
        string url;
    }
    ...
    function rpc(string calldata urlOraliasName, string calldata method, string calldata params)
        external
    ...
    /// Returns a Chain struct for specific aliasName
    function getChain(string calldata chainaliasName) external view returns (Chain memory chain);
    ...
    /// Returns the RPC url for the given aliasName.
    function rpcUrl(string calldata rpcaliasName) external view returns (string memory json);

    /// Returns all rpc urls and their aliasNamees as structs.
    function rpcUrlStructs() external view returns (Rpc[] memory urls);

    /// Returns all rpc urls and their aliasNamees `[aliasName, url][]`.
...
interface Vm is VmSafe {
    ...
    function createFork(string calldata urlOraliasName) external returns (uint256 forkId);

    /// Creates a new fork with the given endpoint and block and returns the identifier of the fork.
    function createFork(string calldata urlOraliasName, uint256 blockNumber) external returns (uint256 forkId);

    /// Creates a new fork with the given endpoint and at the block the given transaction was mined in,
    /// replays all transaction mined in the block before the transaction, and returns the identifier of the fork.
    function createFork(string calldata urlOraliasName, bytes32 txHash) external returns (uint256 forkId);

    /// Creates and also selects a new fork with the given endpoint and the latest block and returns the identifier of the fork.
    function createSelectFork(string calldata urlOraliasName) external returns (uint256 forkId);

    /// Creates and also selects a new fork with the given endpoint and block and returns the identifier of the fork.
    function createSelectFork(string calldata urlOraliasName, uint256 blockNumber) external returns (uint256 forkId);

    /// Creates and also selects new fork with the given endpoint and at the block the given transaction was mined in,
    /// replays all transaction mined in the block before the transaction, returns the identifier of the fork.
    function createSelectFork(string calldata urlOraliasName, bytes32 txHash) external returns (uint256 forkId);
